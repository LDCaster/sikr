        ...
        <div class="datatable-dashv1-list custom-datatable-overright">
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header header-color-modal bg-color-1">
                            <h4 class="modal-title">Tambah Jenis Variant</h4>
                            <div class="modal-close-area modal-close-df">
                                <a class="close" data-dismiss="modal" href="#"><i class="fa fa-close"></i></a>
                            </div>
                        </div>
                        <div class="modal-body">
                            <form action="{{ url('/jenis-variant') }}" method="POST">
                                @csrf
                                <div class="mb-3">
                                    <label for="nama_jenis" class="form-label">Jenis Variant</label>
            ...
            <div id="ModalEdit" class="modal modal-edu-general default-popup-PrimaryModal fade" role="dialog">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header header-color-modal bg-color-1">
                            <h4 class="modal-title">Ubah Jenis Variant</h4>
                            <div class="modal-close-area modal-close-df">
                                <a class="close" data-dismiss="modal" href="#"><i class="fa fa-close"></i></a>
                            </div>
                        </div>
            ...
            <div class="container">
                <h1 style="margin-left: 15px; font-family: 'Roboto Slab', serif;">Data Jenis Variant</h1>
                <hr>
                ...
                <section class="menu-menu">
                    ...
                    <div class="col-sm-10">
                        <form action="{{ url('/jenis-variant-import') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="col-sm-5">
                ...
                <table id="table" data-toggle="table" data-pagination="true" data-search="true" data-show-columns="true"
                    data-show-pagination-switch="false" data-show-refresh="true" data-key-events="true"
                    ...
                    <tbody>
                        @foreach ($jvariants as $jvariant)
                            <tr>
                                <td>
                                    <!-- Button trigger modal -->
                                    <button class="btn btn-warning btn-sm edit" value="{{ $jvariant->id }}"
                                        data-toggle="modal" data-target="#ModalEdit">
                                        <i class="fa-regular fa-pen-to-square"></i>
                                    </button>
                                    <form class="d-inline" style="display: inline"
                                        action="{{ url('/jenis-variant', $jvariant->id) }}" method="POST">
                                        @method('delete')
                                        @csrf
                                        <button type="submit" class="btn btn-danger btn-sm"
                                            onclick="return confirm('Apakah anda yakin untuk hapus data Jenis Variant?')">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.edit').click(function() {
                const id = $(this).val()
                $.ajax({
                    url: `{{ url('/jenis-variant/${id}/edit') }}`,
                    method: "get",
                    success: function(data) {
                        $('#nama_jenis').val(data.nama_jenis)
                        $('#ModalEdit form').attr('action', `{{ url('jenis-variant/${id}') }}`)
                    }
                })
            })
        })
